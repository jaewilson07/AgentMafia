---
url: https://api.slack.com/enterprise/migration
session_id: slack_api_docs
updated_dt: 2025-03-27T00:24:13.072411
---
[](https://api.slack.com/)
...
If you have an existing single-workspace app ready for Enterprise Grid, you may want to make it available for organization-wide deployment.
Read on for tips to keep in mind to make the transition from single-workspace to organization graceful.
## Opt into organization-wide deployment [](https://api.slack.com/enterprise/migration#opt)[](https://api.slack.com/enterprise/migration#opt)
Specify in your [app settings](https://api.slack.com/apps) that your app may be deployed to an entire Enterprise org:
  1. Find the **Org Level Apps** section in the app settings sidebar and press the **Opt-in** button.


That's it! Your app can now be installed by an Org Admin or Owner at the _organization_ level, rather than on an individual workspace.
## Use a minimum version of the SDKs [](https://api.slack.com/enterprise/migration#sdks)[](https://api.slack.com/enterprise/migration#sdks)
If you make use of [Bolt frameworks](https://api.slack.com/tools) to build Slack apps, make sure your version supports organization-ready apps.
...
## Token storage [](https://api.slack.com/enterprise/migration#tokens)[](https://api.slack.com/enterprise/migration#tokens)
...
Previously, you might have stored one row in a database for each installation, and each row might contain a token. Now, you can change your schema to reflect a one-to-many relationship between tokens and installations.
...
## Tokens from previous installations [](https://api.slack.com/enterprise/migration#previous)[](https://api.slack.com/enterprise/migration#previous)
If your app is installed on an entire org, where previously it was installed for specific users and workspaces, you might still have thousands of existing user tokens. _These tokens will continue to function after you become an organization-ready app_.
...
And don't worry: your existing paths to gain user tokens (like [Sign in with Slack](https://api.slack.com/docs/sign-in-with-slack), or just a regular OAuth sign-in flow for users) will still work. Those permissions will each be added to your organization-wide user token, rather than creating a new token.
...
## User IDs [](https://api.slack.com/enterprise/migration#users)[](https://api.slack.com/enterprise/migration#users)
If you previously had **User ID translation** `ON` in your Enterprise app, now is the time to turn it `OFF`.
Single enterprise user IDs are now available. There's no benefit to continuing to maintain local IDs. Use [`migration.exchange`](https://api.slack.com/methods/migration.exchange) to obtain the new single IDs for any users that you have remaining local IDs for.
## Inherited scopes [](https://api.slack.com/enterprise/migration#scopes)[](https://api.slack.com/enterprise/migration#scopes)
When your app is install across an org, the scopes set during that installation pass down to each installation on individual workspaces within that org. Even if you pass different scopes during the [OAuth redirect](https://api.slack.com/authentication/oauth-v2) when you gain access to an individual workspace, you'll still receive only the scopes set during the organization-wide install.
## Webhooks cannot be used [](https://api.slack.com/enterprise/migration#webhooks)[](https://api.slack.com/enterprise/migration#webhooks)
[Webhooks](https://api.slack.com/messaging/webhooks) aren't supported with organization-ready apps. Use `chat.postMessage` and our other [messaging tools](https://api.slack.com/messaging).
## The `chat.postMessage` method requires channel IDs [](https://api.slack.com/enterprise/migration#chat)[](https://api.slack.com/enterprise/migration#chat)
Usually, when you call the [chat.postMessage](https://api.slack.com/methods/chat.postMessage) method to post a message to Slack, you specify the channel using a channel ID.
_If_ , however, you specified the channel using a _channel name_ , the call from your organization-ready app will not succeed. You'll want to begin using channel IDs instead.
## Apps for Admins will already work [](https://api.slack.com/enterprise/migration#admin)[](https://api.slack.com/enterprise/migration#admin)
Some Slack app features that are specifically designed for Enterprise administration, such as the [Audit Logs API](https://api.slack.com/admins/audit-logs), the [SCIM API](https://api.slack.com/admins/scim), and other [Admin APIs](https://api.slack.com/admins), **already** work with Enterprise organizations, rather than individual workspaces.
You only need to install your app at the organization level to use these [Admin APIs](https://api.slack.com/admins) — You won't need to deploy your app across your org.
## On this page
[Opt into organization-wide deployment](https://api.slack.com/enterprise/migration#opt)
[Use a minimum version of the SDKs](https://api.slack.com/enterprise/migration#sdks)
[Token storage](https://api.slack.com/enterprise/migration#tokens)
[Tokens from previous installations](https://api.slack.com/enterprise/migration#previous)
[User IDs](https://api.slack.com/enterprise/migration#users)
[Inherited scopes](https://api.slack.com/enterprise/migration#scopes)
[Webhooks cannot be used](https://api.slack.com/enterprise/migration#webhooks)
[The `chat.postMessage` method requires channel IDs](https://api.slack.com/enterprise/migration#chat)
[Apps for Admins will already work](https://api.slack.com/enterprise/migration#admin)
[Status](https://slack-status.com)[Policy](https://api.slack.com/developer-policy)[Terms](https://slack.com/intl/en-gb/terms-of-service/api-updated)[Your Privacy Choices](https://www.salesforce.com/form/other/privacy-request/ "Your Privacy Choices")[Cookie Preferences](https://api.slack.com/enterprise/migration)
